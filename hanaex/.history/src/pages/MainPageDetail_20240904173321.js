...
const MainPageDetail = () => {
  ...
  useEffect(() => {
    const getUseHistory = async () => {
      try {
        ...
        // setCurrentInvestPrice(result.data.todayData[0].cash_buy);
        // setCurrentSellPrice(result.data.todayData[0].cash_sell);
        // setWonValue(result.data.todayData[0].cash_buy);
        // setVendWonValue(result.data.todayData[0].cash_sell);
        const filteredData = result.data.todayData.filter(
          (item) => item.state === changeState
        );

        if (filteredData.length > 0) {
          const { cash_buy, cash_sell } = filteredData[0];
          setCurrentInvestPrice(cash_buy);
          setCurrentSellPrice(cash_sell);
          setWonValue(cash_buy);
          setVendWonValue(cash_sell);
        }
  ...
  return (
    <div className="w-[960px] bg-slate-500 flex flex-col py-1 px-10">
      ...
      <div className="w-full px-48 py-2 rounded-lg my-2 bg-white">
      ...
      <div className="flex flex-col items-center bg-white rounded-lg py-5">
        {/* 메인 차트 */}
