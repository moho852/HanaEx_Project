...
const MainPageDetail = () => {
  ...
  //     // 트랜잭션 데이터 생성
  //     const fromAccountNumber = selected ? "하나저축예금" : "하나밀리언통장";
  //     const toAccountNumber = selected ? "하나밀리언달러통장" : "하나저축예금";
  //     const withdrawalAmount = selected ? calculatedValue : exchangeValue;
  //     const depositAmount = selected ? exchangeValue : calculatedValue;
...
//     let conclusionStatus = modalType === "DirectModal" ? "completed" : "reserved";
//     let fromAccountNumber = selected ? "하나저축예금" : "하나밀리언통장";
...
//     if (modalType === "DirectModal") {
//         withdrawalAmount = selected ? exchangeValue : calculatedValue;
//         depositAmount = selected ? calculatedValue : exchangeValue;
//     }

//     // 각 통화에 맞게 해당 통화에 값을 할당 (ReserveModal일 경우에는 0으로 유지)
//     let krw = 0, usd = 0, jpy = 0, eur = 0;
//     if (modalType === "DirectModal") {
//         if (changeState === "KRW") {
//             krw = exchangeValue;
//         } else if (changeState === "USD") {
//             usd = exchangeValue;
//         } else if (changeState === "JPY") {
//             jpy = exchangeValue;
//         } else if (changeState === "EUR") {
//             eur = exchangeValue;
//         }
//     }
...
//     // axios 요청으로 서버에 데이터 전송 (DirectModal인 경우)
//     if (modalType === "DirectModal") {
//         axios
//             .post("http://localhost:8082/api/trinsert", transactionData)
//             .then((response) => {
...
//         // ReserveModal인 경우에 추가할 로직
//         const reserveTransactionData = {
//             ...transactionData,
//             withdrawal_amount: 0, // ReserveModal일 경우에는 0
//             deposit_amount: 0,    // ReserveModal일 경우에는 0
...
//           from_account_number: fromAccountNumber,
//           to_account_number: toAccountNumber,
//           withdrawal_amount: 0, // ReserveModal일 경우 0
//           deposit_amount: 0,    // ReserveModal일 경우 0
...
const handlePurchaseClick = () => {
  ...
  let depositAmount = 0;
  let conclusionStatus = "completed"; // 기본값은 "completed", ReserveModal일 때는 "reserved"로 변경

  // 각 통화에 맞게 통화 값 설정
  let krw = 0, usd = 0, jpy = 0, eur = 0;

  // case1 & case2 (DirectModal)
  if (modalType === "DirectModal") {
      if (transactionType === "buy") {
          // case1: Buy + DirectModal
          fromAccountNumber = "하나저축예금";
          toAccountNumber = "하나밀리언달러통장";
          withdrawalAmount = calculatedValue; // KRW에서 출금
          depositAmount = exchangeValue; // 외화로 입금
      } else if (transactionType === "sell") {
          // case2: Sell + DirectModal
          fromAccountNumber = "하나밀리언달러통장";
          toAccountNumber = "하나저축예금";
          withdrawalAmount = exchangeValue; // 외화에서 출금
          depositAmount = calculatedValue; // KRW로 입금
      }
  }

  // case3 & case4 (ReserveModal)
  if (modalType === "ReserveModal") {
      conclusionStatus = "reserved"; // ReserveModal일 때는 항상 "reserved"

      if (transactionType === "buy") {
          // case3: Buy + ReserveModal
          fromAccountNumber = "하나저축예금";
          toAccountNumber = "하나밀리언달러통장";
          withdrawalAmount = calculatedValue; // KRW에서 출금
          depositAmount = exchangeValue; // 외화로 입금
      } else if (transactionType === "sell") {
          // case4: Sell + ReserveModal
          fromAccountNumber = "하나밀리언달러통장";
          toAccountNumber = "하나저축예금";
          withdrawalAmount = exchangeValue; // 외화에서 출금
          depositAmount = calculatedValue; // KRW로 입금
