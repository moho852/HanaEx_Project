...
//       ...selectedTransaction,
//       deposit_amount: updatedExchangeValue,
//       withdrawal_amount: updatedValue,
//     };

//     // 상태 업데이트
//     setTransactionHistory((prevHistory) =>
//       prevHistory.map((item, idx) =>
//         idx === selectedTransaction.index ? updatedTransaction : item
//       )
//     );

//     // 서버로 PUT 요청을 보내 트랜잭션 업데이트
//     axios
//       .put("http://localhost:8082/api/updateTransaction", updatedTransaction)
//       .then((response) => {
...
const MainPageDetail = () => {
  ...
  const handleReservedCardClick = (index) => {
    ...
    // 선택된 거래 정보를 이용하여 외화 구매 금액과 거래 금액을 설정
    setExchangeValue(selectedTransaction.deposit_amount); 
    setCurrentInvestPrice(selectedTransaction.withdrawal_amount); 

    setModalType("ReservedModal");  // ModifyModal을 표시할 준비
    onOpen();  // 모달 열기
  };

  // 트랜잭션 업데이트 후 서버로 PUT 요청을 보내는 함수
  const handleTransactionUpdate = (updatedValue, updatedExchangeValue) => {
    const updatedTransaction = {
      ...selectedTransaction,
      deposit_amount: updatedExchangeValue,
    ...
    <div className="w-[960px] flex flex-col py-1 px-10">
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered={true}>
        ...
        {modalType === "SelectModal" && (
          <SelectModal
            onClose={onClose}
            onPurchaseClick={() => setModalType("FxModal")}
            transactionHistory={transactionHistory}
            handleReservedCardClick={handleReservedCardClick}  // ReservedCard 클릭 핸들러
            changeState={changeState}
            setChangeState={setChangeState}
          />
        )}
        {modalType === "ReservedModal" && selectedTransaction && (
          <ModifyModal
            transaction={selectedTransaction}
            onClose={onClose}
            onUpdate={handleTransactionUpdate}  // 트랜잭션 업데이트 핸들러
          />
        )}
      </Modal>
    </div>
