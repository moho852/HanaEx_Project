...
const MyPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchAccountsData = async () => {
      try {
        const response = await axios.get("/api/getallaccount");
            ...
            <TabPanels>
              <TabPanel>
                ...
                <div className="flex flex-col items-start ml-3">
                  ...
                  <div className="flex items-end mt-2">
                    <Text className="font-semibold leading-0 text-3xl">
                      {/* 계좌 정보 렌더링 (데이터가 있을 경우에만) */}
                      {accountsData ? accountsData[0].krw : '...'} 
                    </Text>
                    <Text className="leading-0 text-2xl"> 원</Text>
                  </div>
                  <Text className="text-slate-600 text-lg">
                    출금가능금액 {accountsData ? accountsData[0].krw : '...'} 원
                  </Text>
                </div>
                  ...
                  {transactionsData ? (
                    transactionsData.map((transaction, index) => (
                      <div key={index}>
                        ...
                        <Text className="text-2xl">{transaction.description}</Text>
                        <Text className="flex text-red-500 justify-end font-semibold text-3xl">
                          {transaction.amount}
                        </Text>
                        <Text className="flex justify-end">{transaction.balance}원</Text>
                        <div className="border bg-slate-600 my-3"></div>
                      </div>
                    ))
                  ) : (
              ...
              <TabPanel>
                {/* 2번째 탭: 보유외환 */}
                <div className="flex flex-col items-start ml-3">
                  <Text className="font-medium text-xl mt-3">하나밀리언달러통장</Text>
                  <Text className="text-slate-600 text-lg">
                    342-910012-87238
                  </Text>
                  <div className="flex items-end my-2">
                    <Text className="leading-0 text-2xl">USD</Text>
                    <Text className="font-semibold leading-0 text-3xl">
                      {accountsData ? accountsData[1].usd : '...'}
                    </Text>
                  </div>
                </div>
              </TabPanel>
