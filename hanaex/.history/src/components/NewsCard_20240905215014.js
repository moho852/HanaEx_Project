...
const NewsCard = ({ state, url, title, content, image_url, result }) => {
  return (
    <Link to={"/NewsPageDetail/" + encodeURIComponent(url)} name="card-container" className='w-[300px] h-[340px] bg-white px-6 pt-3 rounded-2xl drop-shadow-2xl'>
      <div className='flex justify-between pb-4'>
        <Text className='font-medium'>1</Text>
        <Image src='image/usd_flag.png' className='h-[24px]'></Image>
      </div>
